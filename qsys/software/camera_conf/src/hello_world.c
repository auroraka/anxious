...
#define SCCB_DATA CAMERA_SIOD_BASE
#define SCCB_CLK CAMERA_SIOC_BASE
...
volatile unsigned* const sdram = (const unsigned *)0;

volatile unsigned* get_pixel(int bank, int x, int y) {
  return &sdram[(bank << 19) + (x << 10) + y];
}

float limit_in_range(float x, float l, float r) {
  if (x < l) {
    return l;
  } else if (x > r) {
    return r;
...
void ycbcr_to_rgb(unsigned y, unsigned cb, unsigned cr,
    unsigned *r, unsigned *g, unsigned *b) {
  float r_raw, g_raw, b_raw;
  /* b_raw = 2.032 * cb + 1.0 * y; */
  /* g_raw = -0.395 * cb - 0.581 * cr + 1.0 * y; */
  /* r_raw = 1.14 * cr + 1.0 * y; */
  r_raw = y + 1.402 * ((int)cr - 128);
  g_raw = y - 0.34414 * ((int)cb - 128) - 0.71414 * ((int)cr - 128);
  b_raw = y + 1.772 * ((int)cb - 128);
#define F(x) (unsigned)round(limit_in_range((x), 0.0, 255.0))
  *r = F(r_raw);
  *g = F(g_raw);
  *b = F(b_raw);
#undef F
...
int main()
{
  ...
  /* for (;;) { */
    /* for (i = 0; i < kRows; ++i) { */
      /* for (j = 0; j < kCols; ++j) { */
        /* unsigned gbxr = *get_pixel(0, i, j); */
        /* unsigned r, g, b; */
        /* g = ((gbxr >> 12) & 0xF) << 4; */
        /* b = ((gbxr >> 8) & 0xF) << 4; */
        /* r = (gbxr & 0xF) << 4; */
        /* *get_pixel(1, i, j) = PIXEL_FROM_RGB(r, g, b); */
      /* } */
    /* } */
  /* } */
  ...
  for (;0;) {
    for (i = 0; i < kRows; ++i) {
      for (j = 0; j < kCols; ++j) {
        // pixel 1: R[4..0] G[5..3]
        // pixel 2: G[2..0] B[4..0]
        // data = pixel 2 & pixel 1 = G[2..0] B[4..0] R[4..0] G[5..3]
        unsigned data = *get_pixel(0, i, j);
        unsigned r = ((data >> 3) & 0x1F) << 3;
        unsigned g = (((data & 0x7) << 3) | ((data >> 13) & 0x7)) << 2;
        unsigned b = ((data >> 8) & 0x1F) << 3;
        *get_pixel(1, i, j) = PIXEL_FROM_RGB(r, g, b);
      }
    }
    printf("DONE\n");
  ...
  for (;0;) {
    ...
    for (i = 0; i < kRows; ++i) {
      for (j = 0; j < kCols; j += 2) {
        unsigned y0_cb = *get_pixel(0, i, j);
        unsigned y1_cr = *get_pixel(0, i, j + 1);
        unsigned y0, y1, cb, cr;
        unsigned r0, g0, b0;
        unsigned r1, g1, b1;
        y0 = y0_cb & 0xFF;
        y1 = y1_cr & 0xFF;
        ...
        /* *get_pixel(1, i, j) = PIXEL_FROM_RGB(r0, g0, b0); */
        /* *get_pixel(1, i, j + 1) = PIXEL_FROM_RGB(r1, g1, b1); */
        // some-channel-only
        y0 = j >> 2;
        *get_pixel(1, i, j) = PIXEL_FROM_RGB(y0, y0, y0);
        *get_pixel(1, i, j + 1) = PIXEL_FROM_RGB(y0, y0, y0);
        /* ys += y0 + y1; */
        /* cbs += cb * 2; */
        /* crs += cr * 2; */
      }
    }
    /* printf("Ave: y = %.6lf, cb = %.6lf, cr = %.6lf\n", */
        /* ys / kRows / kCols, */
        /* cbs / kRows / kCols, */
        /* crs / kRows / kCols); */
  }
  ...
  /* for (i = 0; i < kRows; ++i) { */
    /* for (j = 0; j < kCols; ++j) { */
      /* data = sdram[(kBank << 19) + (i << 10) + j]; */
      /* r = (data >> 16) & 0xFF; */
      /* g = (data >> 8) & 0xFF; */
      /* b = data & 0xFF; */
      /* if (r != g || r != b) { */
        /* printf("Check error at (%d, %d), data = %x\n", i, j, data); */
      /* } */
      /* if (r == 0 && g == 0 && b == 0) { */
        /* black_cnt++; */
      /* } */
